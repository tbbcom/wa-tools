...
(function (root, factory) {
  ...
  /**
   * Public API
   * StickerTool.init({ root: "#iwaWrapper", debug: false })
   */
  function init(opts) {
    opts = opts || {};
    var rootSel = opts.root || "#iwaWrapper";
    if (!document.querySelector(rootSel)) return; // no container, no-op

    var D = makeDebug(!!opts.debug);
    D.log("init", opts);

    // ===== BEGIN EXTENDED CORE (paste the big code body here) =====
  ...
  function b2img(item) {
    ...
    // Auto-init when the default container exists and user didn't opt out
    if (opts.auto !== false && document.readyState !== "loading") { /* already running inside init */ }
  }

  // Auto-init on DOMContentLoaded if #iwaWrapper exists and user is just <script defer> including it
  if (typeof document !== "undefined") {
    document.addEventListener("DOMContentLoaded", function () {
      var autoNode = document.querySelector("#iwaWrapper");
      if (autoNode) {
        try { init({ root: "#iwaWrapper" }); } catch (e) { /* swallow */ }
      }
    });
  }
